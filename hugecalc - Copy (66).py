...
class HugeComplex:
    ...
    def __pow__(self, exponent_hf) -> 'HugeComplex':
        """Raises the complex number to a non-negative integer power using halving and squaring."""
        if exponent_hf.exponent < 0 or exponent_hf.sign == '-':
            raise ValueError("Complex exponentiation currently only supports non-negative integer powers.")

        # Extract the pure integer value from the HugeFloat exponent
        exp_int = int(exponent_hf.to_dec_string())

        result = HugeComplex('1', '0', self.sig_figs)
        base = self

        # Standard binary exponentiation (halving and squaring)
        while exp_int > 0:
            if exp_int % 2 == 1:
                result = result * base
            base = base * base
...
def _is_huge_digit(s: str) -> bool:
    ...
    # 2. Recursively validate rational fractions
    if '/' in s:
        parts = s.split('/', 1)
    ...
    # 3. Split by 'e' for scientific notation
...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    def str_to_hf(op_str: str):
        ...
        # 2. Intercept and mathematically evaluate fractions recursively
        if '/' in op_str:
    ...
    if op == '+':
        ...
        # Intercept 2D Complex Bases
        if isinstance(hf1, HugeComplex):
            hf2 = str_to_hf(op2)
            if isinstance(hf2, HugeComplex):
                raise ValueError("Complex-to-complex powers are not yet supported.")
            result_hc = hf1 ** hf2
        ...
        # 3. Hybrid Splitting Optimization: b^a = b^int * e^(frac * ln(b))
        one = HugeFloat('1', WORKING_TOL)
        frac_hf = hf2 % one
        int_hf = hf2 - frac_hf
        ...
        if frac_hf.mantissa == '0':
            frac_result = one
        else:
            ln_x = _huge_ln(hf1, WORKING_TOL)
            power_val = frac_hf * ln_x
