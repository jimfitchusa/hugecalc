...
def parse_polynomial(eq_str):
    ...
    # MUST BE INDENTED: Nested helper function
    ...
    # MUST BE INDENTED: Execution and Return
...
def build_dense_array(merged_terms):
    ...
    if not cleaned_terms:
        # Example: 2x + 5 = 2x + 5 (Identity) or 5 = 0 (Contradiction)
        return []

    # 3. Find the true maximum degree
    max_degree = max(cleaned_terms.keys())

    # 4. Build the dense list sequentially from max_degree down to 0
    dense_array = []
    for i in range(max_degree, -1, -1):
        dense_array.append(cleaned_terms.get(i, 0.0))
...
def generate_dk_guesses(dense_array):
    ...
    lead_coef = dense_array[0]
    norm_coefs = [c / lead_coef for c in dense_array]
...
def dk_solve(equation_template, initial_guesses, hctol):
    ...
    Args:
        equation_template (str): The formatted polynomial string (e.g., "1 * {x} ^ 2 + -9").
    ...
    for iteration in range(max_iterations):
        next_guesses = []
        max_error = 0.0  # Optional tracking for terminal output
