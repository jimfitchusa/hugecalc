...
class HugeComplex:
    ...
    def __str__(self) -> str:
        """Formats the complex number dynamically based on HCFORMAT."""
        global HCFORMAT
...
if __name__ == "__main__":
    ...
    if sys.stdout.isatty():
        # --- 1. DATA FORMATTING ---
        if ' * e^(' in raw_result and raw_result.endswith('i)'):
            # Parse and round Polar coordinates
            r_str, theta_str = raw_result.split(' * e^(')
            theta_str = theta_str[:-2]  # Strip 'i)'
            ...
            formatted_result = f"{r_fmt} * e^({theta_fmt}i)"

        elif 'i' in raw_result.lower():
