...
PREFIX_OPERATORS = ('sin', 'cos', 'sind', 'cosd', 'tan', 'tand', 'ln', 'log10', 'exp')
POSTFIX_OPERATORS = ('!',)
...
Example: python hugecalc.py 3 ^ 12      (Integer Power -- note that ^ is the actual operator\nbut in cmd windows environments the ^ operator has to be escapted)
...
Example: python hugecalc.py cos 3.1 | python hugecalc.py round 10     (cos of 3.1 radians rounded to 10 significant digits)

Supported operations: {', '.join(VALID_OPERATORS)}
"""
...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        # 2. Negative Base Fallback (y * ln(x) cannot handle x < 0)
        if hf1.sign == '-':
            force_neg = False
            if '/' in op2:
                num_str, den_str = op2.split('/', 1)
                ...
                hf2 = str_to_hf(num_str) / den_hf
            else:
                hf2 = str_to_hf(op2)
                
            # Route directly to legacy engine
            result_hf = hf1.__pow__(hf2, force_neg)
            return op, str(result_hf), None
