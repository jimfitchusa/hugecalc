...
def _huge_sqrt(mag: str) -> str:
  ...
  $$x_{i+1} = x_i + \frac{c - x_i^2}{2x_i}$$If we do a little bit of algebra to combine those terms by finding a common denominator ($2x_i$), watch what happens to the squared term:$$x_{i+1} = \frac{2x_i^2}{2x_i} + \frac{c - x_i^2}{2x_i}$$$$x_{i+1} = \frac{2x_i^2 + c - x_i^2}{2x_i}$$$$x_{i+1} = \frac{x_i^2 + c}{2x_i}$$$$x_{i+1} = \frac{1}{2} \left(x_i + \frac{c}{x_i} \right)$$This final simplified equation is exactly what next_x = (current_x + mag / current_x) / 2 is calculating in Python! It is the exact same mathematical curve, but by simplifying the algebra, we completely eliminate the need to calculate $x_i^2$ on every single loop, making the engine run significantly faster.
    ...
    # A number with L digits has a square root with roughly L/2 digits.
    guess_len = len(mag) // 2
...
def _huge_powerf(base_mag: str, binary_fraction: str, tol: int) -> str:
    """
    Calculates the fractional power of a base using its binary exponent string.
    Returns the pure integer representation scaled to 'tol' decimal places.
    """
    if binary_fraction == '0':
        # Base^0 = 1, scaled to tol decimal places
        return '1' + ('0' * tol)
        
    # The binary fraction string (e.g., '101') was generated such that the last bit is always '1'.
    # We start by initializing our running total with the square root for that final '1' bit.
    # To get 'tol' decimal places in the square root, we pad the base with 2*tol zeros.
    padded_base = base_mag + ('0' * (2 * tol))
    current_t = _huge_sqrt(padded_base)
    
    # Iterate backwards through the remaining bits (excluding the last one we just processed)
    ...
    for bit in reversed(bits_to_process):
        if bit == '1':
            ...
            # Pad with tol zeros so the square root maintains exactly 'tol' decimals
            padded_val = product + ('0' * tol)
            current_t = _huge_sqrt(padded_val)
        else:
            # t_next = sqrt(t_current)
            
            # Pad with tol zeros so the square root maintains exactly 'tol' decimals
            padded_val = current_t + ('0' * tol)
            current_t = _huge_sqrt(padded_val)
...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        if dec_places2 > 0:
            # We now allow dec_places1 > 0, so we only block decimal exponents
            raise ValueError("Power is only implemented for integer exponents currently.")
            
        # 3. Perform integer exponentiation on the pure magnitudes
        result = huge_power(pure_mag1, pure_mag2)
        
        # 4. Re-insert the decimal point
        # A decimal base raised to an integer exponent multiplies the decimal places
        total_decimal_places = dec_places1 * int(pure_mag2)
        result = _insert_decimal(result, total_decimal_places)
        
        # 5. Determine sign (negative base ^ odd exponent = negative result)
        result_sign = '+'
        if sign1 == '-':
            # If the last digit of the exponent is odd, the result stays negative
            last_digit = int(pure_mag2[-1])
            if last_digit % 2 != 0:
                result_sign = '-'
