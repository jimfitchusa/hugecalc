...
def _insert_decimal(magnitude_str: str, decimal_places: int) -> str:
    ...
    # Example: magnitude "5", decimal_places 2 -> needs to become "005" before insertion
    ...
    result = magnitude_str[:insert_pos] + '.' + magnitude_str[insert_pos:]
...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        try:
            # 3. Perform floating-point division using HCTOL
            # Notice we pass the global HCTOL variable here
            result, appended_decimals = _huge_dividef(pure_mag1, pure_mag2, HCTOL)
            
            # 4. Calculate the final decimal placement
            # Base difference of input decimals + the decimals appended during division
            final_dec_places = (dec_places1 - dec_places2) + appended_decimals
            
            # 5. Insert the decimal (if final_dec_places is negative, we need to right-pad zeros)
        ...
        # We no longer return a remainder for floating point division
        remainder = None
