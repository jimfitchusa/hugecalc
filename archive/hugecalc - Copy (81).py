...
def _huge_gamma(z_hf: HugeFloat, guard_tol: int) -> HugeFloat:
    ...
    coeffs = _huge_spouge_coefficients(a_val, guard_tol)
    ...
    # 2. Prefix terms
    a_hf = HugeFloat(str(a_val), guard_tol)
    z_plus_a = z_hf + a_hf

    half = HugeFloat('0.5', guard_tol)
    z_plus_half = z_hf + half

    # (z + a)^(z + 0.5)
    prefix_pow = z_plus_a ** z_plus_half

    # e^-(z + a) mathematically equals 1 / e^(z + a)
    exp_z_plus_a = _huge_exp(z_plus_a, guard_tol)
    one = HugeFloat('1', guard_tol)
    prefix_exp = one / exp_z_plus_a

    # 3. Final Product
    return prefix_pow * prefix_exp * sum_hf
...
def _huge_complex_gamma(z_hc: HugeComplex, guard_tol: int) -> HugeComplex:
    ...
    # 1. Summation: c_0 + sum(c_k / (z + k))
    sum_hc = HugeComplex(coeffs[0], '0', guard_tol)
    for k in range(1, a_val):
        k_hc = HugeComplex(str(k), '0', guard_tol)
        denom = z_hc + k_hc

        c_k_hc = HugeComplex(coeffs[k], '0', guard_tol)
        term = c_k_hc / denom
        sum_hc = sum_hc + term

    # 2. Prefix terms
    a_hc = HugeComplex(str(a_val), '0', guard_tol)
    z_plus_a = z_hc + a_hc

    half_hc = HugeComplex('0.5', '0', guard_tol)
    z_plus_half = z_hc + half_hc

    # (z + a)^(z + 0.5)
    prefix_pow = z_plus_a ** z_plus_half

    # e^-(z + a) mathematically equals 1 / e^(z + a)
    exp_z_plus_a = _huge_complex_exp(z_plus_a, guard_tol)
    one_hc = HugeComplex('1', '0', guard_tol)
    prefix_exp = one_hc / exp_z_plus_a

    # 3. Final Product
    return prefix_pow * prefix_exp * sum_hc
