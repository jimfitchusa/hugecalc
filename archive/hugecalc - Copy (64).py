...
if __name__ == "__main__":
    ...
    if sys.stdout.isatty():
        # Intercept 2D Complex results to bypass 1D formatting
        if ' * e^(' in raw_result and raw_result.endswith('i)'):
            ...
            print(f"     POLAR: {r_fmt} * e^({theta_fmt}i)")
            ...
            hc_rounded = HugeComplex(real_hf, imag_hf, HCTOL)
            print(f"   COMPLEX: {hc_rounded}")

        else:
            # 1. Determine if the mathematical operation was exact or floating-point
            is_exact = True
            if original_op in ('sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'sind', 'cosd', 'tand', 'asind', 'acosd', 'atand', 'ln', 'log10', 'exp', '/', 'sinh', 'cosh', 'tanh', 'asinh', 'acosh', 'atanh', 'polar', 'rect'):
                ...
                # Basic math becomes inexact if either operand is explicitly a float
                if any(char in op1.lower() for char in ('.', 'e')) or any(char in op2.lower() for char in ('.', 'e')):
                    is_exact = False

            # 2. Pass raw result through HugeFloat
            final_hf = HugeFloat(raw_result, HCTOL)

            # 3. Calculate true scientific exponent and force SCI if precision is exhausted
            sci_exp = final_hf.exponent + len(final_hf.mantissa) - 1
            force_sci = (not is_exact) and (sci_exp >= 15)

            # 4. Physically override integer protections to wipe garbage digits
