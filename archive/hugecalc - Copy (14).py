...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        if dec_places1 > 0 or dec_places2 > 0:
            raise ValueError("Power is only implemented for integers currently.")
            
        # 3. Perform integer exponentiation
        result = huge_power(pure_mag1, pure_mag2)
        
        # 4. Determine sign (negative base ^ odd exponent = negative result)
        result_sign = '+'
        if sign1 == '-':
