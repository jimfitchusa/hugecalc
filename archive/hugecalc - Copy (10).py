...
"""
21 Aug 2026
Working to pick up something next -- perhaps floating point division
...
def parse_and_validate():
    ...
    if len(sys.argv) != 4:
        ...
        if len(sys.argv) == 3 and sys.stdin.isatty() is False:
            ...
            # CHANGED: Print error to stderr
            print("Error: Expected three arguments (operand1, operation, operand2) or piped input.", file=sys.stderr)
            usage()
    else:
    ...
    if not _is_huge_digit(operand1):
        # CHANGED: Print error to stderr
        print(f"Error: Operand 1 ('{operand1}') must be an integer string.", file=sys.stderr)
        usage()

    if not _is_huge_digit(operand2):
        # CHANGED: Print error to stderr
        print(f"Error: Operand 2 ('{operand2}') must be an integer string.", file=sys.stderr)
        usage()
...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        # Division: The quotient sign is negative if signs are different, positive otherwise.
        if (sign1 == '+' and sign2 == '+') or (sign1 == '-' and sign2 == '-'):
            result_sign = '+'
        else:
            result_sign = '-'
        ...
        try:
            result, remainder = huge_divide(mag1, mag2)
    ...
    # 3. Apply the correct sign to the result magnitude (quotient)
    if result == '0':
        # Zero is never negative, regardless of the determined result_sign
        return op, '0', remainder
        
    if result.startswith('-'):
        # This occurs if huge_sub returns a negative sign (only possible in +/- routes)
        # We assume for huge_divide, the result is a non-negative magnitude
        return op, result, remainder 
    
    # Only prepend the determined result_sign if it is negative ('-').
    final_result = result
    if result_sign == '-':
        final_result = '-' + result
    
    # Return the original/effective operator, the final result string (quotient), and remainder
