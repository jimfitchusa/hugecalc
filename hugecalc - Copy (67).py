...
def calculate(op1: str, op: str, op2: str) -> tuple[str, str, str | None]:
    ...
    if op == '+':
        ...
        # 1. Intercept ANY Complex Operations (Base or Exponent)
        if isinstance(hf1, HugeComplex) or isinstance(hf2, HugeComplex):
            if not isinstance(hf1, HugeComplex): hf1 = HugeComplex(hf1, '0', WORKING_TOL)
            if not isinstance(hf2, HugeComplex): hf2 = HugeComplex(hf2, '0', WORKING_TOL)
            result_hc = hf1 ** hf2
        ...
        # 2. Intercept negative real bases with fractional powers
        ...
        # 3. Standard Legacy Root Routing
        ...
        # 3. Zero Base Shortcut
        if hf1.mantissa == '0':
            return op, '0e0', None

        # 4. Negative Base Fallback (for odd roots strictly passed as rational fractions like 1/3)
        if hf1.sign == '-':
            result_hf = _route_legacy_pow(hf1, op2)
            return op, str(result_hf), None
        ...
        # Calculate exact integer portion using high-speed legacy halving/squaring
        if int_hf.mantissa == '0':
            int_result = one
        else:
            int_result = _route_legacy_pow(hf1, str(int_hf))

        # Calculate fractional portion using Maclaurin range-reduction
